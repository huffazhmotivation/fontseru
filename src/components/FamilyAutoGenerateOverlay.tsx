import { memo, useEffect, useMemo, useState, type ChangeEvent, type MouseEvent } from "react";
import { Italic, Layers, Sparkles, Wand2, X } from "lucide-react";
...
function resultMessage(label: string, result: FamilyGenerationResult): string {
...
function FamilyVectorRowImpl({
  ...
  // Defaults to Uppercase instead of "all categories": with multilingual
  // glyphs enabled a family can hold several hundred entries, and each tab
  // added here (Regular/Bold/Italic + every custom family) renders its own
  // full row, so starting unfiltered gets expensive fast. Uppercase mirrors
  // the Live Preview strip below and is still one click away from any
  // other category.
  const [activeCategory, setActiveCategory] = useState<GlyphCategory | null>("upper");
...
// A Family panel with 6 tabs re-renders this row 6× on every parent state
// change (e.g. dragging a Generate slider) unless memoized — each row's
// own props (style/label/glyphs) only actually change when that family's
// glyph data changes, so memoizing keeps drag interactions smooth.
const FamilyVectorRow = memo(FamilyVectorRowImpl);

function LivePreviewStrip({ label, glyphs, hasSource }: { label: string; glyphs: Glyph[]; hasSource: boolean }) {
  return (
    <div className="fm-family-auto-live-preview fm-family-auto-live-preview--strip" aria-label={`${label} live preview, Uppercase A to Z`}>
...
export function FamilyAutoGenerateOverlay() {
  ...
  const generateFamilyCustom = useAppStore((s) => s.generateFamilyCustom);

  // Every Glyph tab that can appear in the Family panel + Generate selector,
  // in display order: Regular, Bold, Italic, then user-created families —
  // the same order new custom tabs are appended in, so the panel grows
  // downward automatically as families are added.
  const allTabs = useMemo<FamilyTab[]>(
    () => [
      ...FONT_STYLES.map(({ id, label }) => ({ id, label })),
      ...customFamilies.map((f) => ({ id: f.id, label: f.name })),
    ],
    [customFamilies]
  );

  const [generateTarget, setGenerateTarget] = useState<FontStyle>("bold");
  ...
  const [customBoldAmount, setCustomBoldAmount] = useState(16);
  const [customItalicAngle, setCustomItalicAngle] = useState(6);
  const [replaceCustom, setReplaceCustom] = useState(false);
  const [status, setStatus] = useState<string | null>(null);

  // If the currently selected Generate target tab was removed (custom
  // family deleted), fall back to Bold instead of pointing at nothing.
  useEffect(() => {
    if (!allTabs.some((t) => t.id === generateTarget)) setGenerateTarget("bold");
  }, [allTabs, generateTarget]);
  ...
  const customPreviewGlyphs = useMemo<Glyph[]>(
    () => upperRegularGlyphs.map((g) =>
      hasOutline(g)
        ? { ...g, outline: autoItalicOutline(autoBoldOutline(g.outline, customBoldAmount), customItalicAngle) }
        : g
    ),
    [upperRegularGlyphs, customBoldAmount, customItalicAngle]
  );
  ...
  const runCustom = (id: FontStyle, label: string) => {
    const result = generateFamilyCustom(id, customBoldAmount, customItalicAngle, replaceCustom);
    setStatus(resultMessage(label, result));
  };

  const activeTab = allTabs.find((t) => t.id === generateTarget) ?? allTabs[1] ?? allTabs[0];
  const isCustomTarget = Boolean(activeTab) && activeTab.id !== "regular" && activeTab.id !== "bold" && activeTab.id !== "italic";
    ...
    >
      <div className="fm-lab-modal fm-family-auto-modal" role="dialog" aria-modal="true" aria-labelledby="family-auto-title">
        ...
        <div className="fm-family-auto-body">
          <div className="fm-family-auto-previews" data-testid="family-panel-previews">
            {allTabs.map(({ id, label }) => (
              <FamilyVectorRow key={id} style={id} label={label} glyphs={glyphsByStyle[id] ?? {}} />
            ))}
          </div>

          <section className="fm-family-generate" aria-label="Generate Preview">
            <div className="fm-family-generate-head">
              <Wand2 size={14} />
              <div>
                <strong>Generate Preview</strong>
                <span>Pick a family, then generate it from Regular</span>
              </div>
            </div>

            <div className="fm-family-generate-tabs" role="tablist" aria-label="Family to generate" data-testid="generate-family-tabs">
              {allTabs.map(({ id, label }) => (
                <button
                  key={id}
                  type="button"
                  role="tab"
                  aria-selected={generateTarget === id}
                  className={generateTarget === id ? "active" : ""}
                  onClick={() => setGenerateTarget(id)}
                  data-testid={`generate-family-tab-${id}`}
                >
                  {label}
                </button>
              ))}
            ...
            {activeTab?.id === "bold" && (
              <div className="fm-family-generate-panel" data-testid="generate-panel-bold">
                <LivePreviewStrip label="Bold" glyphs={boldPreviewGlyphs} hasSource={hasUpperVectors} />
                <Slider
                  label="Bold Amount"
                  value={boldAmount}
                  min={0}
            ...
            {activeTab?.id === "italic" && (
              <div className="fm-family-generate-panel" data-testid="generate-panel-italic">
                <LivePreviewStrip label="Italic" glyphs={italicPreviewGlyphs} hasSource={hasUpperVectors} />
                <Slider
                  label="Italic Angle"
                  value={italicAngle}
                  min={-20}
            ...
            {isCustomTarget && activeTab && (
              <div className="fm-family-generate-panel" data-testid={`generate-panel-${activeTab.id}`}>
                <LivePreviewStrip label={activeTab.label} glyphs={customPreviewGlyphs} hasSource={hasUpperVectors} />
                <Slider
                ...
                />
                <Slider
                  label="Slant (Italic Angle)"
                  value={customItalicAngle}
                  min={-20}
                  max={20}
                  step={1}
                  onChange={setCustomItalicAngle}
                  format={(value) => `${Math.round(value)}°`}
                />
                <label className="fm-checkbox-row fm-family-auto-replace">
                  <input
                    ...
                    onChange={(event: ChangeEvent<HTMLInputElement>) => setReplaceCustom(event.target.checked)}
                  />
                ...
                <button
                  ...
                  data-testid={`auto-generate-${activeTab.id}-btn`}
                >
                  <Wand2 size={14} />
