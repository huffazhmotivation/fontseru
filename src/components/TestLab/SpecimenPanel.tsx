...
import { classifyKerningPair, kerningQualityColor, kerningQualityLabel } from "@/kerning/kerningQuality";
            ...
            >
              ...
              {!isDragging &&
                placed.map((p, index) => {
                  ...
                  const leftGlyphChar = placed[index - 1]?.char;
                  if (!leftGlyphChar) return null;
                  const quality = classifyKerningPair(
                    glyphs,
                    metrics,
                    leftGlyphChar,
                    p.char,
                    kerningPairs[kerningKey(leftGlyphChar, p.char)] ?? 0
                  );
                  if (!quality) return null;
                  return (
                    <div
                      key={`gap-${wrappedLine.start}-${lineIndex}-${index}`}
                      className={`fm-kern-quality-mark ${quality}`}
                      style={{
                        left: p.x * pxPerUnit,
                        top: 0,
                        height: rowHeight,
                        background: kerningQualityColor(quality),
                      }}
                      title={`${leftGlyphChar}${p.char}: ${kerningQualityLabel(quality)}`}
                      data-testid={`kern-quality-${lineIndex}-${index}`}
                      data-kern-quality={quality}
                    />
                  );
                })}
                ...
                >
                  ...
                  {!isDragging &&
                    placed.map((p, index) => {
                      ...
                      return (
                        <div
                          key={`family-gap-${line.start}-${index}`}
                          className={`fm-kern-quality-mark ${quality}`}
                          style={{
                            left: p.x * pxPerUnit,
                            top: 0,
                          ...
                          title={`${leftGlyphChar}${p.char}: ${kerningQualityLabel(quality)}`}
                          data-testid={`family-kern-quality-${style}-${lineIndex}-${index}`}
                          data-kern-quality={quality}
                        />
                      );
                    })}
