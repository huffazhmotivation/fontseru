...
import { detectWorksheet } from "@/worksheet/detectWorksheet";
...
export function TraceImageOverlay() {
  ...
  // Worksheet Import — Auto Glyph Mapping. Only ever set right after a
  // fresh file is picked, and only when `detectWorksheet` confidently
  // recognizes it. Whenever null (not a worksheet, still detecting, or
  // discarded), every existing manual Trace Image / Import SVG code path
  ...
  async function handleImportSvg(next: File) {
    ...
    setWorksheetResult(null);
    try {
      // Worksheet SVGs identify themselves structurally (their own vector
      // marker shapes) — check first so a recognized worksheet's *existing*
      // vector paths are used directly (no raster tracing) and routed to
      // the review panel instead of the flat "one object per shape" manual
      // results canvas below.
      const worksheet = await detectWorksheet(next);
      if (worksheet) {
        setWorksheetResult(worksheet);
        const found = worksheet.cells.filter((c) => c.status === "detected").length;
        showToast(`Worksheet SVG terdeteksi: ${found} sel siap diimpor.`);
        return;
      }
  ...
  // Bulk-commits a set of already-detected worksheet cells. Reuses exactly
  // the same fit + commit path a manual single-letter Apply uses — a
  // worksheet cell's `objects` is just another `VectorObject[]` in raw
  // pixel/unit space, nothing worksheet-specific happens here.
  function handleWorksheetImport(cells: WorksheetCellResult[]) {
    if (!worksheetResult) return;
    let applied = 0;
    let skipped = 0;
    for (const cell of cells) {
      if (cell.status !== "detected") continue;
      const target = regularGlyphs[cell.slot.char];
      if (!target) {
        skipped++;
        continue;
      }
      const outline = fitTracedObjectsToGlyph(cell.objects, metrics, target.advanceWidth);
      commitTracedGlyphOutline(cell.slot.char, outline);
      applied++;
    }
    if (applied > 0) {
      showToast(`${applied} glyph berhasil diimpor dari worksheet.${skipped ? ` ${skipped} dilewati (glyph target tidak ditemukan).` : ""}`);
    } else {
      showToast("Tidak ada sel yang diimpor.", "info");
    }
  }

  /** Discards the worksheet result but keeps the same image loaded, falling back to the ordinary manual trace/select flow. */
  function handleDiscardWorksheet() {
    setWorksheetResult(null);
  }

  /** Cancels the worksheet flow entirely — "gajadi", back to the empty Trace Image starting state. */
  function handleBackFromWorksheet() {
    resetForNewImage();
  }
          ...
          <div className="fm-trace-left">
            {worksheetResult ? (
              <WorksheetReviewPanel
                ...
                metrics={metrics}
                regularGlyphs={regularGlyphs}
                onImport={handleWorksheetImport}
                onDiscard={handleDiscardWorksheet}
