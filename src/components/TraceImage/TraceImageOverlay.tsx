...
import { detectWorksheet, type WorksheetDetectionResult } from "@/worksheet/detectWorksheet";
import { WorksheetReviewPanel } from "@/components/TraceImage/WorksheetReviewPanel";
...
export function TraceImageOverlay() {
  ...
  // Worksheet Import / Auto Glyph Mapping — fully separate from the manual
  // trace state above. `worksheetResult` non-null (and `manualOverride`
  // false) is the *only* thing that switches the panel into worksheet
  // review mode; whenever it's null the overlay behaves exactly as before,
  // completely untouched. See src/worksheet/detectWorksheet.ts.
  ...
  const worksheetDetectTokenRef = useRef(0);
  // Set only when a worksheet is detected in an *SVG* import (the plain
  // `importSvgFile` call is skipped in that case) — lets "Gunakan mode
  // manual" still run the normal SVG import the user would otherwise have
  // gotten immediately.
  const pendingSvgFileRef = useRef<File | null>(null);
  const worksheetMode = worksheetResult !== null && !manualOverride;
  ...
  function handleFileSelected(next: File) {
    ...
    // Auto Glyph Mapping detection runs alongside — never blocks the
    // dropzone preview above, and never touches `letterGroups`/manual state
    // at all. If it finds nothing (the overwhelmingly common case: a
    // regular photo/drawing, not a FontSeru worksheet), the panel below
    // just renders exactly as it always has.
    const token = ++worksheetDetectTokenRef.current;
    setIsDetectingWorksheet(true);
    detectWorksheet(next, "raster")
      .then((res) => {
        if (worksheetDetectTokenRef.current !== token) return; // a newer file was picked meanwhile
        if (res) {
          setWorksheetResult(res);
          showToast(`Worksheet terdeteksi: ${res.templateLabel}.`);
        }
      })
      .finally(() => {
        if (worksheetDetectTokenRef.current === token) setIsDetectingWorksheet(false);
      });
  }
  ...
  function resetForNewImage() {
    ...
    worksheetDetectTokenRef.current++; // invalidate any in-flight worksheet detection for the old file
    pendingSvgFileRef.current = null;
    setPreviewUrl((old) => {
      if (old) URL.revokeObjectURL(old);
      return null;
    });
  ...
  /** Commits the user's chosen cells from the current worksheet detection straight to their mapped glyphs — same fit/commit path as a manual per-letter apply, just looped. Then resets the overlay back to its empty upload state, ready for another file. */
  function handleWorksheetImport(cellIds: number[]) {
    if (!worksheetResult) return;
    const idSet = new Set(cellIds);
    let count = 0;
    for (const cell of worksheetResult.cells) {
      if (!idSet.has(cell.cellId) || cell.objects.length === 0) continue;
      const target = regularGlyphs[cell.char];
      if (!target) continue;
      const outline = fitTracedObjectsToGlyph(cell.objects, metrics, target.advanceWidth);
      commitTracedGlyphOutline(cell.char, outline);
      count++;
    }
    showToast(count > 0 ? `${count} glyph berhasil diimpor dari worksheet.` : "Tidak ada glyph yang diimpor.", count > 0 ? "success" : "error");
    resetForNewImage();
  }

  /** Opts out of worksheet mode for the currently loaded file and drops back into the regular manual flow — for a raster image, `file`/`previewUrl` are already set so the controls below just work; for an SVG, the plain vector import that was skipped when the worksheet was detected runs now. */
  function handleUseManualMode() {
    setManualOverride(true);
    const svgFile = pendingSvgFileRef.current;
    pendingSvgFileRef.current = null;
    if (svgFile) importSvgManually(svgFile);
  }
  ...
  /** The exact original "Import SVG" behavior — no worksheet detection, no threshold/detail/invert, just vectors straight into the results canvas. Kept as its own function so both the plain (non-worksheet) path and the worksheet "Gunakan mode manual" fallback can call the same, unmodified logic. */
  async function importSvgManually(next: File) {
  ...
  async function handleImportSvg(next: File) {
    ...
    setIsImportingSvg(true);
    try {
      // If the SVG already contains vector paths, worksheet cells use them
      // directly — no raster tracing at all (see detectSvgWorksheet.ts).
      const worksheet = await detectWorksheet(next, "svg");
      if (worksheet) {
        pendingSvgFileRef.current = next;
        setWorksheetResult(worksheet);
        showToast(`Worksheet SVG terdeteksi: ${worksheet.templateLabel}.`);
        return;
      }
    } catch {
      // Detection failure just means "not a worksheet" — fall through below.
    } finally {
      setIsImportingSvg(false);
    }
    await importSvgManually(next);
  }
    ...
    >
      <div className="fm-lab-modal fm-trace-modal" role="dialog" aria-modal="true" aria-labelledby="trace-title">
        ...
        <div className="fm-trace-body">
          {worksheetMode && worksheetResult ? (
            <div className="fm-worksheet-body">
              <WorksheetReviewPanel
                result={worksheetResult}
                metrics={metrics}
                onImport={handleWorksheetImport}
                onUseManualMode={handleUseManualMode}
              />
            </div>
          ) : (
          <>
          <div className="fm-trace-left">
            {isDetectingWorksheet && (
              <div className="fm-hint fm-worksheet-detecting" data-testid="worksheet-detecting">
                <Loader2 size={12} className="fm-spin" /> Memeriksa apakah ini Worksheet FontSeru…
              </div>
            )}
