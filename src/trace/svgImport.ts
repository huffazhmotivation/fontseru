...
export function parseTransformAttr(value: string | null): Matrix {
...
export interface RawNode { point: Point; handleIn: Point | null; handleOut: Point | null }
...
export function parsePathData(d: string): RawNode[][] {
...
export const DRAWABLE_TAGS = new Set(["path", "rect", "circle", "ellipse", "polygon", "polyline", "line"]);
...
export const SKIP_SUBTREE_TAGS = new Set(["defs", "symbol", "clippath", "mask", "pattern", "title", "desc", "metadata", "style", "filter"]);
...
/**
 * Drops straight-line points that contribute nothing visually — extremely
 * common in Figma/Illustrator exports (especially after "flatten" or a
 * boolean op), which frequently describe a single straight edge as dozens
 * or hundreds of near-duplicate collinear points instead of two. This is
 * the single biggest reason a "complex" pasted vector could carry an
 * enormous node count into the glyph and make every subsequent redraw,
 * drag, or path rebuild slow — the shape itself is rarely that complex,
 * the point count just is.
 *
 * Only ever removes a node that:
 *   - carries no bezier handle in or out (a real curve control point is
 *     never touched, so no curve's shape can change), and
 *   - sits within floating-point noise of the straight line between its
 *     two neighbors (checked with a tolerance scaled to that segment's own
 *     length, so it behaves the same whether the source SVG used tiny
 *     units or thousand-unit artboards).
 * Genuine corners (even ones that happen to look straight-ish) are far
 * enough off that line to survive untouched.
 */
function reduceRedundantNodes(nodes: RawNode[], closed: boolean): RawNode[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (!closed && (i === 0 || i === n - 1)) {
      out.push(node); // never drop a real open-path endpoint
      continue;
    }
    const prev = nodes[(i - 1 + n) % n];
    const next = nodes[(i + 1) % n];
    if (prev.handleOut || next.handleIn) {
      out.push(node); // adjacent to a curve — keep the exact join point
      continue;
    }
    ...
    const t = ((node.point.x - prev.point.x) * dx + (node.point.y - prev.point.y) * dy) / lenSq;
    const projX = prev.point.x + t * dx;
    const projY = prev.point.y + t * dy;
    const dist = Math.hypot(node.point.x - projX, node.point.y - projY);
    const eps = Math.max(1e-6, Math.sqrt(lenSq) * 1e-4);
    if (dist > eps) {
      out.push(node); // a real corner, not just clutter
    }
    // else: collinear & redundant — drop it.
  }
  const minLen = closed ? 3 : 2;
  return out.length >= minLen ? out : nodes;
}

export function transformRawNode(n: RawNode, m: Matrix): RawNode {
  return {
 ...
 */
export function resolveViewportMatrix(el: Element, isNested: boolean): ViewportResolution {
...
export function walkSvgTree(el: Element, parentMatrix: Matrix, out: WalkedShape[]): void {
  const tag = el.tagName.toLowerCase();
...
export function parseSvgMarkup(text: string): SvgImportResult {
  ...
  for (const shape of shapes) {
    const contours: Contour[] = shape.subpaths
      ...
      .map((sp) => reduceRedundantNodes(sp, true))
      .map((sp) => ({
        id: shortId("trace_c"),
        closed: true,
