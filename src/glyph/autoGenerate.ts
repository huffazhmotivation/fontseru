...
/**
 * Generic Regular → custom-family generator. Custom families have no fixed
 * transform of their own, so this combines the same Bold offset + Italic
 * skew used above into a single adjustable style — weight and slant apply
 * independently, and either can be left at 0.
 */
export function generateCustomFromRegular(
  regular: GlyphMap,
  target: GlyphMap,
  boldAmount: number,
  italicAngle: number,
  ...
  return generateStyle(regular, target, replaceExisting, (outline) =>
    autoItalicOutline(autoBoldOutline(outline, boldAmount), italicAngle)
  );
}
