...
const WOFF_SIGNATURE$1 = 2001684038;
let zlibDecompress = null;
let browserDecompress = null;
function tryLoadZlib$1() {
	try {
...
function tryLoadBrowserDeflate$1() {
	try {
...
zlibDecompress = tryLoadZlib$1();
browserDecompress = tryLoadBrowserDeflate$1();
async function decompress$1(data) {
...
async function woffDecode(data) {
	...
	if (view.getUint32(0) !== WOFF_SIGNATURE$1) throw new Error("Invalid WOFF signature");
	...
	const decompressed = await Promise.all(tables.map(async (table) => {
		...
		const result = await decompress$1(tableData);
...
const SFNT_HEADER_SIZE$2 = 12;
const SFNT_ENTRY_SIZE$2 = 16;
...
async function woffEncode(data, options) {
	...
	const tables = [];
	for (let i = 0; i < numTables; i++) {
		const offset = SFNT_HEADER_SIZE$2 + i * SFNT_ENTRY_SIZE$2;
		tables.push({
	...
	let totalSfntSize = SFNT_HEADER_SIZE$2 + numTables * SFNT_ENTRY_SIZE$2;
	for (const t of tables) totalSfntSize += pad4$1(t.length);
	let woffLength = WOFF_HEADER_SIZE + numTables * WOFF_ENTRY_SIZE;
	for (const c of compressed) woffLength += pad4$1(c.compLength);
	...
	for (let i = 0; i < numTables; i++) {
		...
		dataOffset += pad4$1(c.compLength);
	}
	return output;
}
...
const SFNT_HEADER_SIZE$1 = 12;
const SFNT_ENTRY_SIZE$1 = 16;
const FLAG_ON_CURVE = 1;
const FLAG_X_SHORT$1 = 2;
const FLAG_Y_SHORT$1 = 4;
const FLAG_REPEAT$1 = 8;
const FLAG_X_SAME = 16;
const FLAG_Y_SAME = 32;
const FLAG_OVERLAP_SIMPLE$1 = 64;
...
function computeOffsetToFirstTable(header) {
	if (header.ttcFonts.length === 0) return SFNT_HEADER_SIZE$1 + SFNT_ENTRY_SIZE$1 * header.numTables;
	let offset = 12;
	offset += 4 * header.ttcFonts.length;
	if (header.headerVersion === 131072) offset += 12;
	for (const ttcFont of header.ttcFonts) {
		offset += SFNT_HEADER_SIZE$1;
		offset += SFNT_ENTRY_SIZE$1 * ttcFont.tableIndices.length;
	}
	return offset;
}
...
function writeOffsetTable(view, offset, flavor, numTables) {
	...
	return offset + SFNT_HEADER_SIZE$1;
}
function writeTableEntry(view, offset, tag) {
	...
	return offset + SFNT_ENTRY_SIZE$1;
}
...
function encodeTripletsToScratch(flagStream, glyphStream, nPoints, hasOverlapBit, flagsOut, xOut, yOut) {
	...
	for (let i = 0; i < nPoints; i++) {
		...
		if (hasOverlapBit && i === 0) outFlag |= FLAG_OVERLAP_SIMPLE$1;
		if (dx === 0) outFlag |= FLAG_X_SAME;
		else if (dx >= -255 && dx <= 255) {
			outFlag |= FLAG_X_SHORT$1;
			if (dx > 0) outFlag |= FLAG_X_SAME;
		...
		if (outFlag === lastFlag && repeatCount < 255) {
			flagsOut[flagsLen - 1] |= FLAG_REPEAT$1;
			repeatCount++;
		} else {
			if (repeatCount > 0) {
...
//#endregion
export { woff2Decode, woff2Encode, woffDecode, woffEncode };